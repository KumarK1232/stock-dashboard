...
def tb_fetch_report_data(regime_data=None, sector_data=None, quality_results=None):
    ...
    data = {"timestamp":ts, "date":dt, "macro":{}, "sectors":{}, "cross_assets":{},
            ...
            "sector_strengths": sector_data or {},
            "etf_liq_in":[], "etf_liq_out":[]}
    ...
    for th in _RPT_SECULAR:
        info = _r_proc(th["etf"]); data["secular_prices"][th["etf"]] = info
    # Key levels
...
def tb_generate_report_html(data):
    """Build the 10-section HTML report."""
    ...
    h += "<div style='color:#a0c4ff;font-size:12px;margin-top:4px'>Generated by TopBottom Scanner v2.0 + 7-Layer Quality Filter</div>"
    ...
    h += "<table><tr><th>Indicator</th><th>Price</th><th>Trend</th><th>5d Change</th><th>RSI</th><th>Signal</th></tr>"
    ...
    h += "<table><tr><th>ETF</th><th>Sector</th><th>Price</th><th>5d Trend</th><th>RS vs SPY</th><th>Rating</th></tr>"
    for etf, info in sorted(data.get("sectors",{}).items(), key=lambda x: x[1].get("rs",0), reverse=True):
        ...
        h += "<tr><td><b>%s</b></td><td>%s</td><td>$%s</td><td style='color:%s'>%s%%</td><td style='color:%s;font-weight:bold'>%s%%</td><td><span class='badge %s'>%s</span></td></tr>" % (
            etf, info.get("name",""), _rv(info.get("price")), _rcolor(info.get("trend_5d")), _rv(info.get("trend_5d"),"%+.2f"),
            _rcolor(rs), _rv(rs,"%+.3f"), bc, st)
    h += "</table></div>"
    ...
    # S5: Quality Filter Results (replaces Options section)
    ...
    if qr:
        ...
        for r in qr[:25]:
            gc = "bg" if r.get("quality_grade")=="A+" else ("bg" if r.get("quality_grade")=="A" else "by")
    ...
    for t, lv in data.get("key_levels",{}).items():
        s=lv.get("support"); r2=lv.get("resistance"); c2=lv.get("current")
        if s and r2 and c2 and (r2-s)>0:
            pp=(c2-s)/(r2-s)*100; pt="Near Support" if pp<30 else ("Near Resistance" if pp>70 else "Mid-Range"); pc="#27ae60" if pp<30 else ("#e74c3c" if pp>70 else "#f39c12")
        else: pt="N/A"; pc="#666"
        h += "<tr><td><b>%s</b></td><td class='g'>$%s</td><td><b>$%s</b></td><td class='r'>$%s</td><td style='color:%s;font-weight:bold'>%s</td></tr>" % (t,_rv(s),_rv(c2),_rv(r2),pc,pt)
    h += "</table></div>"
...
def tb_check_and_send_reports(regime_data=None, sector_data=None, quality_results=None):
    """Non-blocking report scheduler. Call after scan completes."""
    ...
    if not report_2pm_sent and now.hour >= REPORT_2PM_HOUR:
        logger.info("  REPORT: 2 PM trigger — generating mid-day report...")
        try:
    ...
    # 4 PM EOD combined report
    if not report_4pm_sent and now.hour >= REPORT_4PM_HOUR:
        logger.info("  REPORT: 4 PM trigger — generating EOD report...")
        try:
            rdata = tb_fetch_report_data(regime_data, sector_data, quality_results)
            html = tb_generate_report_html(rdata)
