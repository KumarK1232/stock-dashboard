...
# 1. FIX: Inbox logic now tags immediately and cleans price strings robustly.
# 2. NEW: Added "Cross 20", "Building Trend", and "Bounce off Yearly Low" logic.
# 3. FIX: Added EMA50 calculation (was missing in compute_indicators but used in scoring).
...
# -------------------- CONFIG --------------------
SCRIPT_VERSION = "vFinal30-InboxFixed"
...
def compute_indicators(df:pd.DataFrame)->pd.DataFrame:
    ...
    d['ema50'] = d['Close'].ewm(span=50, adjust=False).mean() # ADDED
    d['ema21'] = d['Close'].ewm(span=21, adjust=False).mean() # Kept for legacy
    delta = d['Close'].diff()
...
def analyze_ticker(ticker:str, entry_data: Optional[Dict[str, Any]] = None) -> Optional[Dict[str,Any]]:
    try:
        ...
        # --- AI SCORING & CHART GENERATION ---
        # Calculate score early so we can use it for logic tags
        buy_score = calculate_ai_score(daily_ind)
        
        # Generate the interactive chart HTML immediately
        ...
        if entry_data:
            ...
            try:
                ...
                if entry_price > 0:
                    ...
                    if pnl_pct >= 0.05: tags.append("INBOX_PROFIT")
                    elif pnl_pct <= -0.05: tags.append("INBOX_LOSS")
                    if last_close > entry_price and "BREAKOUT_UP" in tags: tags.append("INBOX_BREAKOUT_SUCCESS")
                    if last_close < entry_price and "BREAKOUT_DOWN" in tags: tags.append("INBOX_BREAKDOWN")
            except Exception: pass
        ...
        if len(daily_closes) >= 2:
            ...
            # --- NEW INDICATORS: Cross 20, Building Trend, Yearly Low Bounce ---
            # 1. Cross 20 EMA
            if len(daily_ind) >= 2:
                last_ema20 = daily_ind['ema20'].iloc[-1]
                prev_ema20 = daily_ind['ema20'].iloc[-2]
                prev_close = daily_closes[-2]
                if prev_close < prev_ema20 and last_close > last_ema20: tags.append("CROSS_ABOVE_20EMA")
                if prev_close > prev_ema20 and last_close < last_ema20: tags.append("CROSS_BELOW_20EMA")
                    
            # 2. Building Trend (Price > EMA20 > EMA50)
            if 'ema20' in daily_ind and 'ema50' in daily_ind:
                e20 = daily_ind['ema20'].iloc[-1]
                e50 = daily_ind['ema50'].iloc[-1]
                if last_close > e20 and e20 > e50: tags.append("BUILDING_TREND")

            # 3. Bounce off Yearly Low
            if len(daily_closes) > 20:
                ...
                if year_low > 0:
                    ...
                    if 0 <= dist_from_low <= 0.05: tags.append("BOUNCE_YEARLY_LOW")
        ...
        if len(daily_closes) >= 21:
            ...
            if last_close > prev20_high: tags.append("BREAKOUT_UP")
            if last_close < prev20_low: tags.append("BREAKOUT_DOWN")
        ...
        result = {
            ...
            "buy_score": buy_score,
            "chart_html": chart_html  # New key for the HTML chart
        }
        if entry_data:
...
def generate_light_interactive_chart(df: pd.DataFrame, ticker: str, ai_score: float) -> str:
    """
...
def generate_html_page(page_type, data_groups, outpath, nav_link, source_info, timestamp_str, report_js_template, existing_favorites=None):
    ...
    # --- UPDATED BUTTONS FOR NEW TAGS ---
    tech_btns = [
        ('INTRADAY_TOP', 'Intraday Top'), 
        ('INTRADAY_BOTTOM', 'Intraday Bottom'), 
        ('DAILY_TOP', 'Daily Top'), 
        ('DAILY_BOTTOM', 'Daily Bottom'),
        ('BOUNCE_YEARLY_LOW', 'Bounce Yearly Low'), # NEW
        ('BUILDING_TREND', 'Building Trend'), # NEW
        ('CROSS_ABOVE_20EMA', 'Cross > 20'), # NEW
        ('CROSS_BELOW_20EMA', 'Cross < 20'), # NEW
        ('QUARTERLY_UP', 'Quarter Up'), # ADDED
        ('QUARTERLY_DOWN', 'Quarter Down'), # ADDED
        ('NINE_DAY_HIGH', '9-day High'), 
        ('NINE_DAY_LOW', '9-day Low'), 
    ...
    for tag_id, label in tech_btns:
        style = ""
        if any(x in tag_id for x in ['TOP', 'UP', 'HIGH', 'OVERSOLD', 'BOUNCE', 'BUILDING', 'ABOVE']): style = "background-color:#f0fff4;color:#2f855a;border-color:#c6f6d5;"
        elif any(x in tag_id for x in ['BOTTOM', 'DOWN', 'LOW', 'OVERBOUGHT', 'BELOW']): style = "background-color:#fff5f5;color:#c53030;border-color:#fecaca;"
