...
def build_universe(limit:int=UNIVERSE_LIMIT)->Dict[str,List[str]]:
    ...
    try:
        ...
        logger.warning("Dynamic discovery skipped: %s", e)
...
def main():
    ...
    # GitHub Pages safety: keep docs folder tracked and disable Jekyll for static HTML.
    try:
        os.makedirs(MASTER_OUTPUT_DIR, exist_ok=True)
        open(os.path.join(MASTER_OUTPUT_DIR, ".gitkeep"), "a", encoding="utf-8").close()
        open(os.path.join(MASTER_OUTPUT_DIR, ".nojekyll"), "a", encoding="utf-8").close()
    except Exception as _docs_keep_err:
        logger.warning("Could not create GitHub Pages keeper files: %s", _docs_keep_err)
