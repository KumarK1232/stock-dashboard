...
def analyze_ticker(ticker: str, entry_data: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    try:
        ...
        # -------------------------------------------------------
       # -------------------------------------------------------
        # DAILY SIGNALS (UPDATED WITH TTM SCALPER)
        # -------------------------------------------------------
        daily_signal = False
...
# --- HELPER CLASS FOR SECTOR LOGIC (Singleton Pattern) ---
class SectorGenius:
    _instance = None
    
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(SectorGenius, cls).__new__(cls)
            cls._instance._initialize()
        return cls._instance

    def _initialize(self):
        self.CACHE_FILE = "ticker_sectors.json"
        self.last_rank_update = None
        self.top_sectors = [] # List of ETFs (e.g., ['XLK', 'XLF'])
        
        self.name_to_etf = {
            'Technology': 'XLK', 'Consumer Cyclical': 'XLY', 'Financial Services': 'XLF',
            'Communication Services': 'XLC', 'Healthcare': 'XLV', 'Energy': 'XLE',
            'Industrials': 'XLI', 'Consumer Defensive': 'XLP', 'Basic Materials': 'XLB',
            'Utilities': 'XLU', 'Real Estate': 'XLRE'
        }
        self.etf_to_name = {v: k for k, v in self.name_to_etf.items()}
        self.ticker_cache = self._load_cache()
    ...
    def _save_cache(self):
        try:
            with open(self.CACHE_FILE, 'w') as f:
                json.dump(self.ticker_cache, f)
        except Exception as e:
            logger.error(f"Error saving sector cache: {e}")

   
    def update_sector_ranks(self):
            """Downloads ETF data once per day/session to find top 5 sectors."""
            if self.top_sectors and self.last_rank_update and datetime.now() - self.last_rank_update < timedelta(hours=4):
                return

            try:
                etfs = list(self.name_to_etf.values())
                data = yf.download(etfs, period='6m', interval='1d', progress=False)['Adj Close']
                
                if len(data) > 63:
                    # Performance over the last quarter (approx 63 trading days)
                    perf = (data.iloc[-1] / data.iloc[-63]) - 1
                    # CHANGE: head(5) for Top 5 Sectors
                    self.top_sectors = perf.sort_values(ascending=False).head(5).index.tolist()
                    
                    # --- NEW: DASHBOARD LOGGING ---
                    readable_top = [f"{self.etf_to_name.get(etf, etf)} ({etf})" for etf in self.top_sectors]
                    logger.info("="*50)
                    logger.info("📊 SECTOR STRENGTH DASHBOARD")
                    for i, sec in enumerate(readable_top, 1):
                        logger.info(f"  {i}. {sec}")
                    logger.info("="*50)
                    
                    self.last_rank_update = datetime.now()
            except Exception as e:
                logger.error(f"Failed to update sector ranks: {e}")
                self.top_sectors = list(self.name_to_etf.values())
    def get_ticker_sector(self, ticker):
        """Returns the ETF symbol for the ticker's sector, fetching from YF if unknown."""
        if ticker in self.ticker_cache:
            return self.ticker_cache[ticker]

        # Fetch from Yahoo Finance
        try:
            # logger.info(f"🌍 Fetching sector info for {ticker}...")
            info = yf.Ticker(ticker).info
            raw_sector = info.get('sector', 'Unknown')
            etf = self.name_to_etf.get(raw_sector, 'UNKNOWN')
            
            self.ticker_cache[ticker] = etf
            self._save_cache()
            return etf
        except Exception as e:
    ...
    def worker():
        while True:
            ...
            try:
                ...
                if result:
                    # 1. Try to get the data from the result
                    df = result.get('df') 
                    
                    # FALLBACK: If df is missing, download it manually so we don't skip the scan
                    if df is None:
                        # Fetching 1 year of data to ensure technicals (EMA50) work correctly
                        df = yf.download(ticker, period="1y", interval="1d", progress=False)
                        df.ticker = ticker # Attach ticker name for the sector manager

                    # 2. Run the Sector + Technical check
                    # This now handles 'UNKNOWN' sectors by moving to technicals automatically
                    if df is not None and not df.empty:
                        if detect_buying_opportunity(df, "1d", ticker_name=ticker):
                            if "tags" not in result:
                                result["tags"] = []
                            if "BUYING_OPPORTUNITY" not in result["tags"]:
                                result["tags"].append("BUYING_OPPORTUNITY")
                    else:
                        print(f"⚠️ {ticker}: Skip - Could not obtain price data.")
                    ...
                    # Final Confirmation to Terminal
                    if "BUYING_OPPORTUNITY" in result.get("tags", []):
                         # Using logger.info + print to guarantee terminal visibility
                         msg = f"✨ Buying opportunity confirmed: {ticker}"
                         logger.info(msg)
                         print(msg)

            except Exception as e:
                # Direct print ensures you see worker errors even if logger is suppressed
