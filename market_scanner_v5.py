#!/usr/bin/env python3
"""
MARKET SCANNER v5.9 — PHASE 1+2+3+4+5+6 UPGRADE + DYNAMIC UNIVERSE + SMART GATES
...
STATE_FILE      = "./scanner_state.json"   # v5.7: State persistence for crash recovery
RETENTION_DAYS  = 5     # v5.7: Reduced from 10 for PythonAnywhere 100 MB disk limit
BATCH_CHUNK_SIZE = 40   # v5.3: download in chunks

# v5.9: Dynamic universe config
DYNAMIC_UNIVERSE_SIZE = 500
DEAD_TICKER_THRESHOLD = 3
DEAD_TICKER_BLACKLIST_FILE = "./dead_tickers.json"
...
# ═══════════════════════════════════════════════════════════════
# v5.7 — DYNAMIC HOT STOCKS FETCHER (Yahoo Finance Screeners)
# Pulls today's most active, biggest gainers/losers at runtime
# ═══════════════════════════════════════════════════════════════


# v5.9 — DEAD TICKER AUTO-PRUNE
def load_dead_ticker_blacklist():
    global dead_ticker_blacklist
    try:
        if os.path.exists(DEAD_TICKER_BLACKLIST_FILE):
            with open(DEAD_TICKER_BLACKLIST_FILE, "r") as f:
                data = json.load(f)
                today = get_eastern_now().strftime("%Y-%m-%d")
                if data.get("date") == today:
                    dead_ticker_blacklist = set(data.get("tickers", []))
                    logger.info("  DEAD-PRUNE: Loaded %d blacklisted tickers", len(dead_ticker_blacklist))
                else:
                    dead_ticker_blacklist = set()
                    logger.info("  DEAD-PRUNE: New day — reset blacklist")
    except Exception as e:
        logger.warning("  DEAD-PRUNE: Could not load blacklist: %s", e)
        dead_ticker_blacklist = set()


def save_dead_ticker_blacklist():
    try:
        data = {"date": get_eastern_now().strftime("%Y-%m-%d"), "tickers": sorted(list(dead_ticker_blacklist)), "count": len(dead_ticker_blacklist)}
        with open(DEAD_TICKER_BLACKLIST_FILE, "w") as f:
            json.dump(data, f, indent=2)
    except Exception as e:
        logger.warning("  DEAD-PRUNE: Could not save blacklist: %s", e)


def record_ticker_failure(ticker):
    global dead_ticker_tracker, dead_ticker_blacklist
    dead_ticker_tracker[ticker] = dead_ticker_tracker.get(ticker, 0) + 1
    if dead_ticker_tracker[ticker] >= DEAD_TICKER_THRESHOLD:
        if ticker not in dead_ticker_blacklist:
            dead_ticker_blacklist.add(ticker)
            logger.warning("  DEAD-PRUNE: %s BLACKLISTED after %d failures", ticker, dead_ticker_tracker[ticker])
            save_dead_ticker_blacklist()


def record_ticker_success(ticker):
    if ticker in dead_ticker_tracker:
        dead_ticker_tracker[ticker] = 0


def get_dynamic_hot_stocks():
    """v5.9: Enhanced dynamic stock discovery — Yahoo + Finviz + TradingView."""
    hot = set()

    # Method 1: yfinance screeners
    try:
        if hasattr(yf, 'screen'):
            for sn in ['day_gainers', 'day_losers', 'most_actives']:
                try:
                    result = yf.screen(sn, size=50)
                    if result and 'quotes' in result:
                        for q in result['quotes']:
                            sym = q.get('symbol', '')
                            if sym and '.' not in sym and len(sym) <= 5:
                                hot.add(sym.replace('-', ''))
                    logger.info("  DYNAMIC: %s returned %d (total=%d)", sn, len(result.get('quotes',[])) if result else 0, len(hot))
                except Exception as ex:
                    logger.warning("  DYNAMIC: %s failed: %s", sn, ex)
    except Exception as ex:
        logger.warning("  DYNAMIC: yf.screen not available: %s", ex)

    # Method 2: EquityQuery
    try:
        from yfinance import EquityQuery
        q = EquityQuery('and', [EquityQuery('eq', ['region', 'us']), EquityQuery('gt', ['dayvolume', 1000000]), EquityQuery('gt', ['intradaymarketcap', 2000000000]), EquityQuery('gt', ['intradayprice', 5])])
        ...
        if result and 'quotes' in result:
            for item in result['quotes']:
                ...
                if sym and '.' not in sym and len(sym) <= 5:
                    hot.add(sym.replace('-', ''))
        logger.info("  DYNAMIC: EquityQuery returned %d (total=%d)", len(result.get('quotes',[])) if result else 0, len(hot))
    except ImportError:
        logger.info("  DYNAMIC: EquityQuery not available")
    except Exception as ex:
        logger.warning("  DYNAMIC: EquityQuery failed: %s", ex)

    # Method 3: Yahoo trending
    try:
        url = "https://query1.finance.yahoo.com/v1/finance/trending/US?count=50"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 scanner-v5.9"})
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            for q in quotes:
                ...
                if sym and '.' not in sym and len(sym) <= 5:
                    hot.add(sym.replace('-', ''))
        logger.info("  DYNAMIC: Yahoo trending returned %d (total=%d)", len(quotes), len(hot))
    except Exception as ex:
        logger.warning("  DYNAMIC: Yahoo trending failed: %s", ex)

    # Method 4: Yahoo most_actives
    try:
        url2 = "https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved?formatted=false&scrIds=most_actives&count=50"
        req2 = urllib.request.Request(url2, headers={"User-Agent": "Mozilla/5.0 scanner-v5.9"})
        with urllib.request.urlopen(req2, timeout=15) as resp2:
            ...
            for q in quotes2:
                ...
                if sym and '.' not in sym and len(sym) <= 5:
                    hot.add(sym.replace('-', ''))
        logger.info("  DYNAMIC: Yahoo most_actives returned %d (total=%d)", len(quotes2), len(hot))
    except Exception as ex:
        logger.warning("  DYNAMIC: Yahoo most_actives failed: %s", ex)

    # Method 5: Finviz HTML scrape
    finviz_signals = [("ta_topgainers", "Top Gainers"), ("ta_toplosers", "Top Losers"), ("ta_mostactive", "Most Active"), ("ta_unusualvolume", "Unusual Volume"), ("ta_newhigh", "New Highs")]
    for signal_id, signal_name in finviz_signals:
        try:
            fv_url = "https://finviz.com/screener.ashx?v=111&s=%s&f=sh_avgvol_o400,sh_price_o5" % signal_id
            fv_req = urllib.request.Request(fv_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", "Accept": "text/html"})
            with urllib.request.urlopen(fv_req, timeout=20) as fv_resp:
                fv_html = fv_resp.read().decode("utf-8", errors="ignore")
                ticker_matches = re.findall(r'quote[.]ashx[?]t=([A-Z]{1,5})', fv_html)
                seen = set()
                for t in ticker_matches:
                    if t not in seen and len(t) <= 5 and t.isalpha():
                        seen.add(t)
                        hot.add(t)
                logger.info("  DYNAMIC: Finviz %s returned %d (total=%d)", signal_name, len(seen), len(hot))
            time.sleep(1.5)
        except Exception as ex:
            logger.warning("  DYNAMIC: Finviz %s failed: %s", signal_name, str(ex)[:80])

    # Method 5b: Finviz Python library (optional)
    try:
        from finviz.screener import Screener
        for filters, label in [(['sh_avgvol_o400', 'sh_price_o5', 'ta_rsi_os30'], 'oversold'), (['sh_avgvol_o400', 'sh_price_o5', 'ta_rsi_ob70'], 'overbought')]:
            sl = Screener(filters=filters, order='averagevolume')
            for stock in sl[:50]:
                sym = stock.get('Ticker', '')
                if sym and len(sym) <= 5: hot.add(sym)
            logger.info("  DYNAMIC: Finviz lib %s returned %d (total=%d)", label, min(len(sl), 50), len(hot))
        time.sleep(2)
    except ImportError:
        logger.info("  DYNAMIC: finviz library not installed (optional: pip install finviz)")
    except Exception as ex:
        logger.warning("  DYNAMIC: Finviz library failed: %s", str(ex)[:80])

    # Method 6: TradingView screener (optional)
    try:
        from tradingview_screener import Query, Column
        tv_results = (Query().select('name', 'close', 'volume').where(Column('volume') > 500000, Column('close') > 5, Column('market_cap_basic') > 1e9, Column('is_primary') == True).order_by('volume', ascending=False).limit(100).get_scanner_data())
        if tv_results and len(tv_results) > 1:
            df_tv = tv_results[1]
            for _, row in df_tv.iterrows():
                sym = str(row.get('name', ''))
                if sym and len(sym) <= 5 and sym.isalpha(): hot.add(sym)
            logger.info("  DYNAMIC: TradingView returned %d (total=%d)", len(df_tv), len(hot))
    except ImportError:
        logger.info("  DYNAMIC: tradingview-screener not installed (optional: pip install tradingview-screener)")
    except Exception as ex:
        logger.warning("  DYNAMIC: TradingView failed: %s", str(ex)[:80])

    # Remove blacklisted dead tickers
    before = len(hot)
    hot -= dead_ticker_blacklist
    if before > len(hot):
        logger.info("  DYNAMIC: Removed %d dead/blacklisted tickers", before - len(hot))
...
def refresh_scan_universe(scan_number, force=False):
    """v5.9: Refresh dynamic universe, prune dead tickers, cap at DYNAMIC_UNIVERSE_SIZE."""
    ...
    if force or scan_number <= 1 or (scan_number - dynamic_refresh_scan) >= 2:
        ...
        try:
            ...
            logger.warning("  UNIVERSE: Dynamic refresh failed: %s", ex)
    else:
        logger.info("  UNIVERSE: Using cached dynamic tickers (%d) from scan #%d", len(dynamic_hot_tickers), dynamic_refresh_scan)

    combined = list(dict.fromkeys(ALL_STOCKS + list(dynamic_hot_tickers)))
    before_prune = len(combined)
    combined = [t for t in combined if t not in dead_ticker_blacklist]
    pruned = before_prune - len(combined)
    if pruned > 0:
        logger.info("  UNIVERSE: Pruned %d dead tickers: %s", pruned, ", ".join(sorted(dead_ticker_blacklist)[:20]))
    if len(combined) > DYNAMIC_UNIVERSE_SIZE:
        logger.info("  UNIVERSE: Capping from %d to %d", len(combined), DYNAMIC_UNIVERSE_SIZE)
        combined = combined[:DYNAMIC_UNIVERSE_SIZE]
    logger.info("  UNIVERSE: Total=%d (base=%d + dynamic=%d - pruned=%d, cap=%d)", len(combined), len(ALL_STOCKS), len(dynamic_hot_tickers), pruned, DYNAMIC_UNIVERSE_SIZE)
    return combined
...
# v5.7: File-based logging — persists even if PythonAnywhere console disconnects
try:
    from logging.handlers import RotatingFileHandler
    # v5.7: Kept small for PythonAnywhere free tier (100 MB disk limit)
    _fh = RotatingFileHandler("scanner.log", maxBytes=1*1024*1024, backupCount=1)
...
def log_scan_completion_summary(scan_no, scan_time, regime, spy_rsi, time_quality_name,
    ...
    if sector_snap:
        ...
        for sec, s in sorted(sector_snap.items(), key=lambda kv: float(kv[1].get("diff") or 0), reverse=True):
            ...
            # v5.7: Enhanced sector display with visual strength indicator
            ...
            logger.info("")  # v5.9: Blank line between each sector for readability
    else:
    ...
    # ═══════════════════════════════════════════════════════════════
    # v5.7 — NEAR-MISS / HIGH POTENTIAL WATCHLIST
...
def safe_download(ticker, period, interval, max_retries=3):
    """v5.9: Download with freshness, 429 backoff, dead ticker tracking."""
    if ticker in dead_ticker_blacklist:
        return None
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if d is not None and not d.empty:
                ...
                if meta.get("fallback_used") or logger.isEnabledFor(logging.DEBUG):
                    logger.info("  DL %s: usable source=%s latest=%s age=%sm rows=%d interval=%s", ticker, meta.get("source"), meta.get("latest_et"), meta.get("age_min"), len(d), interval)
                return d
            logger.warning("  DL %s #%d unusable: meta=%s", ticker, attempt, meta)
        except Exception as ex:
            err_str = str(ex)
            logger.warning("DL %s #%d fail: %s", ticker, attempt, ex)
            if "429" in err_str or "Too Many Requests" in err_str:
                backoff = attempt * 5 + random.uniform(3, 8)
                logger.warning("  DL %s: 429 rate limit — backing off %.1fs", ticker, backoff)
                time.sleep(backoff)
            elif attempt < max_retries:
                time.sleep(attempt * 3 + random.uniform(1, 3))
    record_ticker_failure(ticker)
...
# (v5.7: 5m+15m+30m STRICT, 60m OPTIONAL BOOSTER, early promotion for high-confidence)
# ═══════════════════════════════════════════════════════
...
def check_multi_timeframe(ticker, market_regime, sector_str):
    ...
    # ═══════════════════════════════════════════════════════════════
    # v5.7 — EARLY PROMOTION: Skip 60M gate for high-confidence stocks
...
def cleanup_disk_space():
    """v5.7: Disk cleanup for PythonAnywhere free tier (100 MB limit).
    Removes old state files, temp files, VACUUMs DB, warns if >50 MB used.
    """
    try:
        today = get_eastern_now().strftime("%Y-%m-%d")
        cleaned = []

        # Clean scanner_state.json if not today's
        if os.path.exists(STATE_FILE):
            try:
                ...
                if state.get("saved_date", "") != today:
                    ...
                    cleaned.append("scanner_state.json(old)")
            except Exception:
                pass

        # Clean temp files
        for tmp in [STATE_FILE + ".tmp"]:
        ...
        # Calculate total disk usage of scanner files
        scanner_files = [
            "scanner.log", "scanner.log.1",
            "scanner_alerts.db", "scanner_alerts.db-wal", "scanner_alerts.db-shm",
            "scanner_state.json", "scanner_heartbeat.txt",
            "market_scanner_v5.py", "flask_app.py",
        ]
        total_bytes = 0
        ...
        if cleaned:
            logger.info("  Disk cleanup: removed %s", ", ".join(cleaned))
        logger.info("  Scanner disk usage: %.1f MB / 100 MB", total_mb)

        if total_mb > 50:
            logger.warning("  Disk usage > 50 MB! Check PythonAnywhere Files tab for cleanup.")

    except Exception as e:
        logger.warning("  Disk cleanup error: %s", e)
...
# ═══════════════════════════════════════════════════════════════
# v5.7 — STATE PERSISTENCE: Save/Restore critical state to disk
# ═══════════════════════════════════════════════════════════════

def save_scanner_state(scan_number=0, crash_count=0):
    """v5.7: Persist critical scanner state to disk for crash recovery."""
    global confirmation_tracker, sent_alerts, daily_alerts
    try:
        now = get_eastern_now()
        ct_serialized = {}
        for ticker, v in confirmation_tracker.items():
            ...
            for k, val in v.items():
                if isinstance(val, datetime):
                    entry[k] = val.isoformat()
                elif hasattr(val, "isoformat") and callable(val.isoformat):
                    entry[k] = val.isoformat()
                else:
                    entry[k] = val
            ct_serialized[ticker] = entry
        da_serialized = []
        for alert in daily_alerts:
            a = {}
            for k, val in alert.items():
                if isinstance(val, (datetime, date)):
                    ...
                    a[k] = val
            da_serialized.append(a)
        state = {
            "saved_at": now.isoformat(),
            "saved_date": now.strftime("%Y-%m-%d"),
            "scan_number": scan_number,
            "scan_crash_count": crash_count,
            "confirmation_tracker": ct_serialized,
            "sent_alerts": list(sent_alerts),
            "daily_alerts": da_serialized,
        }
        tmp = STATE_FILE + ".tmp"
        with open(tmp, "w") as f:
            json.dump(state, f, indent=2, default=str)
        os.replace(tmp, STATE_FILE)
        logger.info("  State saved: %d tracked, %d sent, %d daily, scan #%d",
                     len(confirmation_tracker), len(sent_alerts), len(daily_alerts), scan_number)
    except Exception as e:
        logger.warning("  State save failed: %s", e)


def restore_scanner_state():
    """v5.7: Restore scanner state from disk after crash/restart."""
    global confirmation_tracker, sent_alerts, daily_alerts
    try:
        ...
        today = get_eastern_now().strftime("%Y-%m-%d")
        if saved_date != today:
            logger.info("  State from %s (today=%s) — starting fresh", saved_date, today)
            try:
                os.remove(STATE_FILE)
            except Exception:
                pass
        ...
        try:
            ...
            if saved_at.tzinfo is None:
                saved_at = ET.localize(saved_at)
            age_minutes = (now - saved_at).total_seconds() / 60.0
            if age_minutes > 30:
                stale_tracker = True
                logger.info("  State %.1f min old — tracker too stale", age_minutes)
            else:
                logger.info("  State %.1f min old — valid for restoration", age_minutes)
        except Exception:
            stale_tracker = True
        if not stale_tracker:
            ct = state.get("confirmation_tracker", {})
            confirmation_tracker.clear()
            confirmation_tracker.update(ct)
            logger.info("  Restored confirmation_tracker: %d stocks", len(confirmation_tracker))
            for ticker, v in list(confirmation_tracker.items())[:5]:
                logger.info("     %s: stage=%s count=%s c5=%s", ticker, v.get("stage","?"), v.get("count","?"), v.get("c5","?"))
            if len(confirmation_tracker) > 5:
                logger.info("     ... and %d more", len(confirmation_tracker) - 5)
        else:
            logger.info("  confirmation_tracker NOT restored (%.0f min old)", age_minutes)
        sa = state.get("sent_alerts", [])
        sent_alerts.clear()
        sent_alerts.update(sa)
        logger.info("  Restored sent_alerts: %d", len(sent_alerts))
        da = state.get("daily_alerts", [])
        daily_alerts.clear()
        daily_alerts.extend(da)
        logger.info("  Restored daily_alerts: %d", len(daily_alerts))
        scan_number = state.get("scan_number", 0)
        crash_count = state.get("scan_crash_count", 0)
        logger.info("  Resuming from scan #%d (crashes: %d)", scan_number, crash_count)
        return scan_number, crash_count
    except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════════
# v5.8 — DAILY MARKET INTELLIGENCE REPORT (2 PM + 4 PM ET Auto-Email)
# Non-blocking: runs AFTER each scan completes, never interrupts processing.
# 2 PM = Mid-day Market Intelligence Report
# 4 PM = Combined Market Intelligence + EOD Trading Summary
# ═══════════════════════════════════════════════════════════════════════════

import re as _re_module   # For HTML tag stripping in combined report
...
# ── Instrument Universe for Report ──
MACRO_INSTRUMENTS = {
    "SPY": {"name": "S&P 500", "type": "equity"},
    "QQQ": {"name": "Nasdaq 100", "type": "equity"},
    "IWM": {"name": "Russell 2000", "type": "equity"},
    "DIA": {"name": "Dow 30", "type": "equity"},
}
CROSS_ASSET_INSTRUMENTS = {
    "BTC-USD": {"name": "Bitcoin", "type": "crypto"},
    "ETH-USD": {"name": "Ethereum", "type": "crypto"},
    "SOL-USD": {"name": "Solana", "type": "crypto"},
    "GLD": {"name": "Gold (GLD)", "type": "commodity"},
    "SLV": {"name": "Silver (SLV)", "type": "commodity"},
    "USO": {"name": "Oil (USO)", "type": "commodity"},
    "UNG": {"name": "NatGas (UNG)", "type": "commodity"},
    "COPX": {"name": "Copper (COPX)", "type": "commodity"},
    "URA": {"name": "Uranium (URA)", "type": "commodity"},
    "LIT": {"name": "Lithium (LIT)", "type": "commodity"},
    "TLT": {"name": "20Y Bonds", "type": "bond"},
    "HYG": {"name": "High-Yield", "type": "bond"},
}
VOLATILITY_INSTRUMENTS = {"^VIX": {"name": "VIX", "type": "volatility"}}
DOLLAR_INSTRUMENTS = {"DX-Y.NYB": {"name": "US Dollar", "type": "currency"}}
YIELD_INSTRUMENTS = {
    "^TNX": {"name": "10Y Yield", "type": "yield"},
    "^IRX": {"name": "3M Yield", "type": "yield"},
}
...
SECULAR_THEMES = [
    {"theme": "AI Infrastructure", "tickers": ["NVDA","AVGO","AMD","SMCI"],
     "etf": "SMH", "probability": "85-90%", "timeframe": "2025-2030"},
    {"theme": "Nuclear Renaissance", "tickers": ["CCJ","SMR","NNE","UEC"],
     "etf": "URA", "probability": "75-85%", "timeframe": "2025-2035"},
    {"theme": "Cybersecurity", "tickers": ["CRWD","PANW","FTNT","ZS"],
     "etf": "CIBR", "probability": "80-90%", "timeframe": "2025-2030"},
    {"theme": "Space Economy", "tickers": ["RKLB","ASTS","LUNR","PL"],
     "etf": "UFO", "probability": "60-75%", "timeframe": "2025-2035"},
    {"theme": "EV & Battery", "tickers": ["TSLA","RIVN","QS","ALB"],
     "etf": "LIT", "probability": "70-80%", "timeframe": "2025-2030"},
    {"theme": "Water Infrastructure", "tickers": ["XYL","WMS","WTRG","ECL"],
     "etf": "PHO", "probability": "70-80%", "timeframe": "2025-2035"},
    {"theme": "DeFi / Blockchain", "tickers": ["COIN","HOOD","MARA","RIOT"],
     "etf": "BITO", "probability": "50-65%", "timeframe": "2025-2030"},
    {"theme": "Robotics & Automation", "tickers": ["ISRG","PATH","ABBV"],
     "etf": "ROBO", "probability": "75-85%", "timeframe": "2025-2030"},
    {"theme": "Clean Energy", "tickers": ["FSLR","ENPH","SEDG","NEE"],
     "etf": "ICLN", "probability": "65-80%", "timeframe": "2025-2035"},
    {"theme": "Quantum Computing", "tickers": ["IONQ","RGTI","QBTS","GOOG"],
     "etf": "QTUM", "probability": "40-60%", "timeframe": "2028-2040"},
]


def _rpt_safe_rsi(close_series, period=14):
    try:
        if close_series is None or len(close_series) < period + 2:
            return None
        rsi = calculate_rsi(close_series, period)
        v = rsi.dropna()
        return round(float(v.iloc[-1]), 1) if not v.empty else None
    except Exception:
        return None

def _rpt_trend_pct(close_series, days):
    try:
        if close_series is None or len(close_series) < days + 1:
            return None
        return round((float(close_series.iloc[-1]) - float(close_series.iloc[-days-1])) / float(close_series.iloc[-days-1]) * 100, 2)
    except Exception:
        return None

def _rpt_rvol(vol_series, period=20):
    try:
        if vol_series is None or len(vol_series) < period + 1:
            return None
        avg = vol_series.iloc[-period-1:-1].mean()
        return round(float(vol_series.iloc[-1]) / float(avg), 2) if avg > 0 else None
    except Exception:
        return None

def _rpt_val(v, fmt="%.2f", na="N/A"):
    try:
        return fmt % float(v) if v is not None else na
    except Exception:
        return na
...
def _rpt_color(val, g=0, r=0):
    if val is None: return "#666"
    return "#27ae60" if val > g else ("#e74c3c" if val < r else "#f39c12")


def fetch_report_data():
    """Fetch all market data for the 10-section daily report."""
    logger.info("=" * 65)
    logger.info("  REPORT: Fetching data for Daily Market Intelligence Report...")
    logger.info("=" * 65)

    report = {
        "timestamp": get_eastern_now().strftime("%Y-%m-%d %I:%M:%S %p ET"),
        "date": get_eastern_now().strftime("%Y-%m-%d"),
        "macro": {}, "sectors": {}, "cross_assets": {},
        "volatility": {}, "dollar": {}, "yields": {},
        "secular_prices": {}, "options_pcr": {},
        "alerts_today": list(daily_alerts) if daily_alerts else [],
        "near_misses": list(near_miss_candidates) if near_miss_candidates else [],
        "etf_liquidity_in": [], "etf_liquidity_out": [],
        "key_levels": {},
        "regime": market_regime_cache.get("regime", "NEUTRAL"),
        "spy_rsi": market_regime_cache.get("spy_rsi", 50.0),
    }

    def _proc(df, name):
        info = {"price": None, "change_pct": None, "rsi": None, "trend_5d": None, "trend_20d": None, "rvol": None}
        try:
            if df is None or df.empty: return info
            c = df["Close"].dropna()
            v = df["Volume"].dropna() if "Volume" in df.columns else None
            if c.empty: return info
            info["price"] = round(float(c.iloc[-1]), 2)
            if len(c) >= 2:
                info["change_pct"] = round((float(c.iloc[-1]) - float(c.iloc[-2])) / float(c.iloc[-2]) * 100, 2)
            info["rsi"] = _rpt_safe_rsi(c)
            info["trend_5d"] = _rpt_trend_pct(c, 5)
            info["trend_20d"] = _rpt_trend_pct(c, 20)
            if v is not None and not v.empty:
                info["rvol"] = _rpt_rvol(v)
        except Exception as e:
            logger.warning("  REPORT: Error processing %s: %s", name, e)
        return info

    # Batch download all tickers
    all_tickers = list(dict.fromkeys(
        list(MACRO_INSTRUMENTS) + list(SECTOR_ETFS_FULL) + list(CROSS_ASSET_INSTRUMENTS) +
        list(VOLATILITY_INSTRUMENTS) + list(DOLLAR_INSTRUMENTS) + list(YIELD_INSTRUMENTS) + ["SPY"]
    ))
    logger.info("  REPORT: Downloading %d tickers...", len(all_tickers))

    all_data = {}
    chunks = [all_tickers[i:i+40] for i in range(0, len(all_tickers), 40)]
    for ci, chunk in enumerate(chunks):
        try:
            _reinit_tz_mem_cache()
            bd = safe_yf_download(chunk, period="1mo", interval="1d",
                                  progress=False, auto_adjust=True, threads=False, group_by="ticker")
            if bd is not None and not bd.empty:
                if isinstance(bd.columns, pd.MultiIndex):
                    for t in chunk:
                        try:
                            if t in bd.columns.get_level_values(0):
                                tdf = bd[t].dropna(how="all")
                    ...
                    if len(chunk) == 1:
                        all_data[chunk[0]] = bd.dropna(how="all")
            time.sleep(2)
        except Exception as e:
            logger.warning("  REPORT: Chunk %d failed: %s", ci+1, e)
            time.sleep(3)

    logger.info("  REPORT: Got data for %d/%d tickers", len(all_data), len(all_tickers))

    missing_tickers = [t for t in all_tickers if t not in all_data]
    if missing_tickers:
        logger.warning("  REPORT: Missing data for %d tickers: %s", len(missing_tickers), ", ".join(missing_tickers[:30]))
    if not is_regular_market_hours():
        logger.info("  REPORT: Market CLOSED — using last trading day data")


    # Process macro
    spy_close = None
    for t, meta in MACRO_INSTRUMENTS.items():
        info = _proc(all_data.get(t), t)
        info["name"] = meta["name"]
        report["macro"][t] = info
        if t == "SPY" and all_data.get("SPY") is not None:
            spy_close = all_data["SPY"]["Close"].dropna()

    # Process sectors with RS vs SPY
    for etf, sname in SECTOR_ETFS_FULL.items():
        info = _proc(all_data.get(etf), etf)
        info["name"] = sname
        info["rs_vs_spy"] = None; info["stage"] = "NEUTRAL"; info["rating"] = "NEUTRAL"
        try:
            if spy_close is not None and etf in all_data:
                sc = all_data[etf]["Close"].dropna()
                common = sc.index.intersection(spy_close.index)
                if len(common) >= 6:
                    sc2 = sc.loc[common]; sp2 = spy_close.loc[common]
                    s5 = (float(sc2.iloc[-1]) - float(sc2.iloc[-6])) / float(sc2.iloc[-6]) * 100
                    p5 = (float(sp2.iloc[-1]) - float(sp2.iloc[-6])) / float(sp2.iloc[-6]) * 100
                    rs = round(s5 - p5, 3)
                    info["rs_vs_spy"] = rs
                    t5 = info.get("trend_5d") or 0
                    if rs > 0.3 and t5 > 0: info["stage"], info["rating"] = "LEADING", "STRONG"
                    elif rs > 0 and t5 > 0: info["stage"], info["rating"] = "IMPROVING", "STRONG"
                    elif rs < -0.3 and t5 < 0: info["stage"], info["rating"] = "LAGGING", "WEAK"
                    elif rs < 0: info["stage"], info["rating"] = "WEAKENING", "WEAK"
        except Exception as e:
            logger.warning("  REPORT: Sector RS failed %s: %s", etf, e)
        report["sectors"][etf] = info

    # Cross-assets, volatility, dollar, yields
    for t, meta in CROSS_ASSET_INSTRUMENTS.items():
        info = _proc(all_data.get(t), t); info["name"] = meta["name"]; info["asset_type"] = meta["type"]
        report["cross_assets"][t] = info
    for t, meta in VOLATILITY_INSTRUMENTS.items():
        info = _proc(all_data.get(t), t); info["name"] = meta["name"]; report["volatility"][t] = info
    for t, meta in DOLLAR_INSTRUMENTS.items():
        info = _proc(all_data.get(t), t); info["name"] = meta["name"]; report["dollar"][t] = info
    for t, meta in YIELD_INSTRUMENTS.items():
        info = _proc(all_data.get(t), t); info["name"] = meta["name"]; report["yields"][t] = info

    # Secular prices
    sec_tickers = set()
    for th in SECULAR_THEMES:
        sec_tickers.add(th["etf"])
        for tk in th["tickers"][:2]: sec_tickers.add(tk)
    for t in sec_tickers:
        if t in all_data: report["secular_prices"][t] = _proc(all_data.get(t), t)

    # Options PCR
    for etf in ["XLK", "XLF", "XLE", "XLV", "XLY", "SPY"]:
        try: report["options_pcr"][etf] = round(get_options_pcr(etf), 2)
        except Exception: report["options_pcr"][etf] = None

    # ETF liquidity flows
    thematic_map = {
        "SMH": "Semiconductors", "SOXX": "Semiconductors", "HACK": "Cybersecurity",
        "CIBR": "Cybersecurity", "ARKK": "Innovation", "ARKG": "Genomics",
        "ARKF": "Fintech", "TAN": "Solar", "ICLN": "Clean Energy", "URA": "Uranium",
        "LIT": "Lithium/EV", "BOTZ": "Robotics", "JETS": "Airlines",
        "XBI": "Biotech", "KRE": "Regional Banks", "XOP": "Oil Exploration",
        "BITO": "Bitcoin Futures", "GDX": "Gold Miners", "KWEB": "China Internet",
        "PAVE": "Infrastructure", "GRID": "Grid/Smart Power",
    }
    for etf, theme in thematic_map.items():
        info = _proc(all_data.get(etf), etf)
        rv = info.get("rvol"); t5 = info.get("trend_5d") or 0
        if rv and rv > 1.2 and t5 > 0:
            report["etf_liquidity_in"].append({"etf": etf, "theme": theme, "rvol": rv, "trend_5d": t5, "price": info.get("price")})
        elif rv and rv < 0.8 and t5 < 0:
            report["etf_liquidity_out"].append({"etf": etf, "theme": theme, "rvol": rv, "trend_5d": t5, "price": info.get("price")})

    # Key levels
    for t in ["SPY", "QQQ", "BTC-USD", "GLD", "USO", "TLT"]:
        try:
            if t in all_data:
                df = all_data[t]
                if len(df) >= 5:
                    report["key_levels"][t] = {
                        ...
                        "support": round(float(df["Low"].iloc[-5:].min()), 2),
...
def generate_full_market_report_html(data):
    """Build the full 10-section HTML market intelligence report."""
...
.ctr{max-width:900px;margin:0 auto;background:#fff}
.hdr{background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);color:#fff;padding:25px 30px;text-align:center}
.hdr h1{margin:0;font-size:22px;letter-spacing:1px}
.hdr .sub{color:#a0c4ff;font-size:13px;margin-top:6px}
.hdr .ts{color:#ccc;font-size:11px;margin-top:8px}
.sec{padding:20px 25px;border-bottom:1px solid #e0e0e0}
.sec h2{color:#1a1a2e;font-size:17px;margin:0 0 12px;border-left:4px solid #0f3460;padding-left:10px}
.sec h3{color:#333;font-size:14px;margin:12px 0 6px}
table{width:100%;border-collapse:collapse;margin:8px 0;font-size:12px}
th{background:#1a1a2e;color:#fff;padding:8px 6px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.5px}
td{padding:6px;border-bottom:1px solid #e8e8e8}
tr:nth-child(even){background:#f8f9fa}
.g{color:#27ae60;font-weight:bold}.r{color:#e74c3c;font-weight:bold}.y{color:#f39c12;font-weight:bold}
.badge{display:inline-block;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:bold;color:#fff}
.bg{background:#27ae60}.br{background:#e74c3c}.by{background:#f39c12}.bb{background:#3498db}
.rbx{display:inline-block;padding:6px 16px;border-radius:5px;font-size:14px;font-weight:bold;color:#fff;margin:6px 0}
.ftr{background:#f0f0f0;padding:15px 25px;font-size:10px;color:#888}
</style>"""

    h = "<html><head>" + css + "</head><body><div class='ctr'>"

    # HEADER
    rc = "#27ae60" if regime == "BULLISH" else ("#e74c3c" if regime == "BEARISH" else "#f39c12")
    h += "<div class='hdr'><h1>&#128202; Elite Hedge Fund — Full Market Intelligence Report</h1>"
    h += "<div class='sub'>Daily Institutional-Grade Market Analysis</div>"
    h += "<div class='ts'>Generated: %s &nbsp;|&nbsp; Source: PythonAnywhere Scanner v5.8 + yfinance</div>" % ts
    h += "<div style='margin-top:10px'><span class='rbx' style='background:%s'>Market Regime: %s &nbsp; SPY RSI: %.1f</span></div></div>" % (rc, regime, spy_rsi)

    # S1: Macro
    spy_i = data["macro"].get("SPY", {}); vix_i = data["volatility"].get("^VIX", {})
    dxy_i = data["dollar"].get("DX-Y.NYB", {}); tnx_i = data["yields"].get("^TNX", {}); irx_i = data["yields"].get("^IRX", {})
    vp = vix_i.get("price"); st20 = spy_i.get("trend_20d"); dt5 = dxy_i.get("trend_5d"); tt5 = tnx_i.get("trend_5d")
    econ = "Expansion" if st20 and st20 > 2 else ("Contraction" if st20 and st20 < -2 else "Mid-Cycle")
    risk = "Risk-On" if vp and vp < 18 else ("Risk-Off" if vp and vp > 25 else "Cautious")
    vl = "Low (<15)" if vp and vp < 15 else ("Elevated (>25)" if vp and vp > 25 else "Normal")
    mon = "Tightening" if tt5 and tt5 > 0.5 else ("Easing" if tt5 and tt5 < -0.5 else "Stable")
    ddir = "Strengthening" if dt5 and dt5 > 0.3 else ("Weakening" if dt5 and dt5 < -0.3 else "Range-Bound")
    tp = tnx_i.get("price"); ip = irx_i.get("price")
    yc = ("Normal (+%.2f)" % (tp-ip) if tp-ip > 0 else "Inverted (%.2f)" % (tp-ip)) if tp and ip else "N/A"

    h += "<div class='sec'><h2>&#128200; Section 1: Macro Regime &amp; Market Environment</h2>"
    h += "<table><tr><th>Indicator</th><th>Current</th><th>Trend</th><th>Signal</th><th>Assessment</th></tr>"
    for row in [
        ("Economic Cycle", econ, _rpt_arrow(st20), "SPY 20d: %s%%" % _rpt_val(st20,"%+.1f"), ""),
        ("Risk Appetite", risk, _rpt_arrow(-(vix_i.get("trend_5d") or 0)), "VIX: %s" % _rpt_val(vp,"%.1f"), vl),
        ("Market Regime", regime, "", "SPY RSI: %.1f" % spy_rsi, ""),
        ("US Dollar (DXY)", _rpt_val(dxy_i.get("price"),"%.2f"), _rpt_arrow(dt5), ddir, "5d: %s%%" % _rpt_val(dt5,"%+.1f")),
        ("10Y Yield", _rpt_val(tp,"%.2f%%"), _rpt_arrow(tt5), mon, ""),
        ("Yield Curve 10Y-3M", yc, "", "", ""),
        ("VIX", _rpt_val(vp,"%.2f"), _rpt_arrow(vix_i.get("trend_5d")), _rpt_rsi_signal(vix_i.get("rsi")), vl),
    ]:
        h += "<tr><td><b>%s</b></td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>" % row
    h += "</table></div>"

    # S2: Sectors
    h += "<div class='sec'><h2>&#128260; Section 2: GICS Sector Rotation</h2>"
    h += "<table><tr><th>ETF</th><th>Sector</th><th>Price</th><th>5d Trend</th><th>RS vs SPY</th><th>Stage</th><th>RVOL</th><th>Rating</th></tr>"
    for etf, info in sorted(data["sectors"].items(), key=lambda x: x[1].get("rs_vs_spy") or 0, reverse=True):
        rs = info.get("rs_vs_spy"); rating = info.get("rating","NEUTRAL")
        bc = "bg" if rating=="STRONG" else ("br" if rating=="WEAK" else "by")
        h += "<tr><td><b>%s</b></td><td>%s</td><td>$%s</td>" % (etf, info.get("name",""), _rpt_val(info.get("price")))
        h += "<td style='color:%s'>%s%%</td>" % (_rpt_color(info.get("trend_5d")), _rpt_val(info.get("trend_5d"),"%+.2f"))
        h += "<td style='color:%s;font-weight:bold'>%s%%</td>" % (_rpt_color(rs), _rpt_val(rs,"%+.3f"))
        h += "<td>%s</td><td>%s</td>" % (info.get("stage",""), _rpt_val(info.get("rvol"),"%.2fx"))
        h += "<td><span class='badge %s'>%s</span></td></tr>" % (bc, rating)
    h += "</table></div>"

    # S3: Cross-Asset
    h += "<div class='sec'><h2>&#127757; Section 3: Cross-Asset Dashboard</h2>"
    h += "<table><tr><th>Asset</th><th>Price</th><th>Trend</th><th>5d Chg</th><th>RSI</th><th>Signal</th><th>RVOL</th></tr>"
    all_cross = list(data["cross_assets"].items()) + list(data["volatility"].items()) + list(data["dollar"].items()) + list(data["yields"].items())
    for t, info in all_cross:
        t5 = info.get("trend_5d"); rsi = info.get("rsi")
        sig = _rpt_rsi_signal(rsi)
        sc2 = "#27ae60" if "Bull" in sig or "Oversold" in sig else ("#e74c3c" if "Bear" in sig or "Overbought" in sig else "#666")
        h += "<tr><td><b>%s</b> <small style='color:#888'>%s</small></td>" % (t, info.get("name",""))
        h += "<td>%s</td><td>%s</td>" % (_rpt_val(info.get("price")), _rpt_arrow(t5))
        h += "<td style='color:%s'>%s%%</td><td>%s</td>" % (_rpt_color(t5), _rpt_val(t5,"%+.2f"), _rpt_val(rsi,"%.0f"))
        h += "<td style='color:%s'>%s</td><td>%s</td></tr>" % (sc2, sig, _rpt_val(info.get("rvol"),"%.2fx"))
    h += "</table></div>"

    # S4: Secular Themes
    h += "<div class='sec'><h2>&#128640; Section 4: Secular Growth Themes (10-20X Potential)</h2>"
    h += "<table><tr><th>Theme</th><th>Key Tickers</th><th>ETF</th><th>ETF Price</th><th>5d Trend</th><th>Probability</th><th>Timeframe</th></tr>"
    for th in SECULAR_THEMES:
        ei = data["secular_prices"].get(th["etf"], {}); t5 = ei.get("trend_5d") if ei else None
        h += "<tr><td><b>%s</b></td><td>%s</td><td>%s</td><td>$%s</td>" % (th["theme"], ", ".join(th["tickers"][:4]), th["etf"], _rpt_val(ei.get("price") if ei else None))
        h += "<td style='color:%s'>%s%%</td><td>%s</td><td>%s</td></tr>" % (_rpt_color(t5), _rpt_val(t5,"%+.2f"), th["probability"], th["timeframe"])
    h += "</table></div>"

    # S5: Options
    h += "<div class='sec'><h2>&#128202; Section 5: Options Put/Call Ratio</h2>"
    h += "<table><tr><th>ETF</th><th>P/C Ratio</th><th>Signal</th><th>Interpretation</th></tr>"
    for etf, pcr in data.get("options_pcr", {}).items():
        if pcr is None: sig2, interp2 = "N/A", "Data unavailable"
        elif pcr > 1.2: sig2, interp2 = "<span class='g'>Bullish (Contrarian)</span>", "Heavy put buying - potential bounce"
        elif pcr < 0.7: sig2, interp2 = "<span class='r'>Bearish (Contrarian)</span>", "Heavy call buying - potential pullback"
        else: sig2, interp2 = "<span class='y'>Neutral</span>", "Balanced options flow"
        h += "<tr><td><b>%s</b></td><td>%s</td><td>%s</td><td>%s</td></tr>" % (etf, _rpt_val(pcr,"%.2f"), sig2, interp2)
    h += "</table></div>"

    # S6: Alerts + Near-Miss
    h += "<div class='sec'><h2>&#128293; Section 6: Scanner Alerts &amp; Pre-Breakout Stocks</h2>"
    alerts = data.get("alerts_today", [])
    if alerts:
        h += "<h3>Today's Confirmed Alerts</h3><table><tr><th>Ticker</th><th>Signal</th><th>Confidence</th><th>Price</th><th>Sector</th><th>Setup</th><th>MTF</th></tr>"
        for a in alerts:
            sc3 = "#e74c3c" if a.get("signal")=="TOP" else "#27ae60"
            h += "<tr><td><b>%s</b></td><td style='color:%s'>%s</td><td><b>%s</b></td><td>$%s</td><td>%s</td><td>%s</td><td>%s</td></tr>" % (
                a.get("ticker","?"), sc3, a.get("signal","?"), a.get("avg_c","?"), _rpt_val(a.get("cl")), a.get("ss","?"), a.get("setup_type","?"), a.get("mtf_status","?"))
        h += "</table>"
    else:
        h += "<p style='color:#888'>No confirmed alerts yet today.</p>"
    nms = data.get("near_misses", [])
    if nms:
        h += "<h3>&#128310; Near-Miss Watchlist</h3><table><tr><th>Ticker</th><th>Signal</th><th>C5</th><th>Sector</th><th>Stage</th><th>Price</th></tr>"
        for nm in nms[:15]:
            h += "<tr><td><b>%s</b></td><td>%s</td><td>%s</td><td>%s (%s)</td><td>%s</td><td>$%s</td></tr>" % (
                nm.get("ticker","?"), nm.get("signal","?"), nm.get("c5","?"), nm.get("sector","?"), nm.get("sector_strength","?"), nm.get("stage","?"), _rpt_val(nm.get("last_price")))
        h += "</table>"
    h += "</div>"

    # S7: Liquidity In
    h += "<div class='sec'><h2>&#128176; Section 7: ETFs Attracting Liquidity</h2>"
    li_in = sorted(data.get("etf_liquidity_in", []), key=lambda x: x.get("rvol",0), reverse=True)
    if li_in:
        h += "<table><tr><th>ETF</th><th>Theme</th><th>RVOL</th><th>5d Trend</th><th>Price</th></tr>"
        for li in li_in[:15]:
            h += "<tr><td><b>%s</b></td><td>%s</td><td class='g'>%.2fx</td><td class='g'>%s%%</td><td>$%s</td></tr>" % (
                li["etf"], li["theme"], li["rvol"], _rpt_val(li.get("trend_5d"),"%+.2f"), _rpt_val(li.get("price")))
        h += "</table>"
    else: h += "<p style='color:#888'>No thematic ETFs showing unusual inflow today.</p>"
    h += "</div>"

    # S8: Liquidity Out
    h += "<div class='sec'><h2>&#128682; Section 8: Where Liquidity Is Exiting</h2>"
    li_out = sorted(data.get("etf_liquidity_out", []), key=lambda x: x.get("rvol",1))
    if li_out:
        h += "<table><tr><th>ETF</th><th>Theme</th><th>RVOL</th><th>5d Trend</th><th>Price</th></tr>"
        for lo in li_out[:15]:
            h += "<tr><td><b>%s</b></td><td>%s</td><td class='r'>%.2fx</td><td class='r'>%s%%</td><td>$%s</td></tr>" % (
                lo["etf"], lo["theme"], lo["rvol"], _rpt_val(lo.get("trend_5d"),"%+.2f"), _rpt_val(lo.get("price")))
        h += "</table>"
    else: h += "<p style='color:#888'>No thematic ETFs showing unusual outflow today.</p>"
    h += "</div>"

    # S9: Key Levels
    h += "<div class='sec'><h2>&#127919; Section 9: Key Institutional Price Levels</h2>"
    h += "<table><tr><th>Instrument</th><th>5d Support</th><th>Current</th><th>5d Resistance</th><th>Position</th></tr>"
    for t, lvl in data.get("key_levels", {}).items():
        sup=lvl.get("support"); res=lvl.get("resistance"); cur=lvl.get("current")
        if sup and res and cur and (res-sup) > 0:
            pp = (cur-sup)/(res-sup)*100
            pt = "Near Support" if pp<30 else ("Near Resistance" if pp>70 else "Mid-Range")
            pc = "#27ae60" if pp<30 else ("#e74c3c" if pp>70 else "#f39c12")
        else: pt, pc = "N/A", "#666"
        h += "<tr><td><b>%s</b></td><td class='g'>$%s</td><td><b>$%s</b></td><td class='r'>$%s</td>" % (t, _rpt_val(sup), _rpt_val(cur), _rpt_val(res))
        h += "<td style='color:%s;font-weight:bold'>%s</td></tr>" % (pc, pt)
    h += "</table></div>"

    # S10: Forecast
    vv = vix_i.get("price") or 20; st5 = spy_i.get("trend_5d") or 0; st20v = spy_i.get("trend_20d") or 0
    strong_s = sum(1 for s in data["sectors"].values() if s.get("rating")=="STRONG")
    weak_s = sum(1 for s in data["sectors"].values() if s.get("rating")=="WEAK")
    forecasts = []
    if regime=="BULLISH" and vv<20: forecasts.append(("1 Week","Bullish","75%","Momentum + low VIX","#27ae60"))
    elif regime=="BEARISH": forecasts.append(("1 Week","Bearish","65%","Negative momentum","#e74c3c"))
    else: forecasts.append(("1 Week","Neutral","55%","Mixed signals","#f39c12"))
    if st20v>2: forecasts.append(("1 Month","Bullish","70%","Strong 20d uptrend","#27ae60"))
    elif st20v<-2: forecasts.append(("1 Month","Bearish","65%","20d downtrend","#e74c3c"))
    ...
    elif weak_s>strong_s+3: forecasts.append(("3 Months","Bearish","60%","Sector weakness broadening","#e74c3c"))
    else: forecasts.append(("3 Months","Neutral","50%","Sector rotation ongoing","#f39c12"))
    forecasts += [("6 Months","Cautiously Bullish","60%","Secular themes provide support","#27ae60"),
                  ("1 Year","Bullish","65%","Innovation cycle favors equities","#27ae60"),
                  ("3-5 Years","Strongly Bullish","75-85%","AI, energy transition, deglobalization tailwinds","#27ae60")]

    h += "<div class='sec'><h2>&#128302; Section 10: Multi-Timeframe Forecast</h2>"
    h += "<table><tr><th>Timeframe</th><th>Direction</th><th>Confidence</th><th>Key Driver</th></tr>"
    ...
    # FOOTER
    h += "<div class='ftr'>"
    h += "<p><b>Legend:</b> RS=Relative Strength vs SPY | RVOL=Relative Volume vs 20d avg | RSI>70=Overbought | RSI<30=Oversold</p>"
    h += "<p><b>Source:</b> yfinance via PythonAnywhere Scanner v5.8 | Real-time market data</p>"
    h += "<p><b>Disclaimer:</b> For informational purposes only. Not financial advice.</p>"
    h += "<p>Auto-generated at %s</p></div>" % ts
    h += "</div></body></html>"
    return h


def _build_combined_eod_report(intel_html, eod_html, data):
    """Combine Market Intelligence + EOD Summary into one email."""
    ts = data.get("timestamp", "Unknown")
    combined = "<html><head><style>"
    combined += "body{font-family:'Segoe UI',Arial,sans-serif;margin:0;padding:0}"
    combined += ".divider{background:linear-gradient(135deg,#1a1a2e,#0f3460);color:#fff;padding:15px 25px;text-align:center;margin:30px 0 0}"
    combined += ".divider h2{margin:0;font-size:18px}.divider .sub{color:#a0c4ff;font-size:12px;margin-top:4px}"
    combined += "</style></head><body>"

    # Part 1: Strip html/body from intel report and embed
    ib = _re_module.sub(r'<html.*?<body[^>]*>', '', intel_html, flags=_re_module.DOTALL)
    ib = _re_module.sub(r'</body>.*?</html>', '', ib, flags=_re_module.DOTALL)
    combined += ib

    # Divider
    combined += "<div class='divider'><h2>&#128203; Part 2: End-of-Day Trading Summary</h2>"
    combined += "<div class='sub'>Alerts, P&amp;L, Win/Loss Analysis - %s</div></div>" % ts

    # Part 2: Strip html/body from EOD and embed
    if eod_html:
        eb = _re_module.sub(r'<html.*?<body[^>]*>', '', eod_html, flags=_re_module.DOTALL)
        eb = _re_module.sub(r'</body>.*?</html>', '', eb, flags=_re_module.DOTALL)
        combined += eb
    else:
        combined += "<div style='padding:20px;color:#888'>No alerts were generated today.</div>"
...
def check_and_send_daily_report():
    """
    Non-blocking report scheduler. Runs AFTER each scan completes.
    Checks if it's time for the 2 PM or 4 PM report.
    Uses separate flags so each fires exactly once per day.
    """
    global report_2pm_sent, report_4pm_sent
    now = get_eastern_now()

    # ── 2:00 PM Mid-Day Market Intelligence Report ──
    if not report_2pm_sent and now.hour >= REPORT_2PM_HOUR:
        logger.info("  DAILY REPORT: 2 PM trigger - generating mid-day report...")
        try:
            rpt_data = fetch_report_data()
            html = generate_full_market_report_html(rpt_data)
            subject = "Mid-Day Market Intelligence - %s" % rpt_data.get("date", now.strftime("%Y-%m-%d"))
            if SEND_EMAIL and EMAIL_ADDRESS and EMAIL_PASSWORD and RECIPIENT_EMAIL:
                msg = MIMEMultipart("alternative")
                msg["Subject"] = subject
                msg["From"] = EMAIL_ADDRESS
                ...
                logger.info("  DAILY REPORT: 2 PM report sent to %s", ", ".join(RECIPIENT_EMAIL))
            else:
                logger.warning("  DAILY REPORT: Email not configured. 2 PM report generated but not sent.")
            report_2pm_sent = True
        except Exception as e:
            logger.exception("  DAILY REPORT: 2 PM report failed: %s", e)

    # ── 4:00 PM End-of-Day Combined Report ──
    if not report_4pm_sent and now.hour >= REPORT_4PM_HOUR:
        logger.info("  DAILY REPORT: 4 PM trigger - generating EOD combined report...")
        try:
            rpt_data = fetch_report_data()
            intel_html = generate_full_market_report_html(rpt_data)
            eod_html = build_eod_report()
            combined_html = _build_combined_eod_report(intel_html, eod_html, rpt_data)
            subject = "End-of-Day Combined Report - %s" % rpt_data.get("date", now.strftime("%Y-%m-%d"))
            if SEND_EMAIL and EMAIL_ADDRESS and EMAIL_PASSWORD and RECIPIENT_EMAIL:
                msg = MIMEMultipart("alternative")
                msg["Subject"] = subject
                msg["From"] = EMAIL_ADDRESS
                ...
                with smtplib.SMTP(SMTP_SERVER, 587) as server:
                    ...
                    server.login(EMAIL_ADDRESS, EMAIL_PASSWORD)
                    server.sendmail(EMAIL_ADDRESS, RECIPIENT_EMAIL, msg.as_string())
                logger.info("  DAILY REPORT: 4 PM combined report sent to %s", ", ".join(RECIPIENT_EMAIL))
            else:
                logger.warning("  DAILY REPORT: Email not configured. 4 PM report generated but not sent.")
            report_4pm_sent = True
        except Exception as e:
            logger.exception("  DAILY REPORT: 4 PM combined report failed: %s", e)
...
def main():
    ...
    logger.info("=" * 65)
    logger.info("MARKET SCANNER v5.9 — PHASE 1+2+3+4+5+6 + DYNAMIC UNIVERSE + SMART GATES")
    ...
    init_db()
    load_dead_ticker_blacklist()  # v5.9

    # v5.7: Restore state from disk if available (crash recovery)
    _restored_scan, _restored_crashes = restore_scanner_state()

    # v5.7: Clean disk space (PythonAnywhere 100 MB limit)
    cleanup_disk_space()

    # v5.7 SELF-HEALING: Wait for market hours instead of exiting
    now = get_eastern_now()
    ...
    # v5.8: Reset daily report flags for new trading day
    global report_2pm_sent, report_4pm_sent
    report_2pm_sent = False
    report_4pm_sent = False
    sc = _restored_scan          # v5.7: Resume from last scan after crash

    scan_crash_count = _restored_crashes  # v5.7: Preserve crash count
    ...
    while True:
        ...
        try:
            ...
            # v5.7: Write heartbeat file for monitoring
            try:
                with open("scanner_heartbeat.txt", "w") as _hb:
                    _hb.write("alive|scan#%d|%s\n" % (sc, now.strftime("%Y-%m-%d %H:%M:%S ET")))
            except Exception:
                pass

            # v5.7 HEARTBEAT: Every 5 scans, confirm the scanner is alive
            if sc % 5 == 0:
                logger.info("═" * 65)
            ...
            # v5.7: Persist state to disk after each successful scan
            try:
                save_scanner_state(sc, scan_crash_count)
            except Exception as state_err:
                logger.warning("  State save failed: %s", state_err)

            # v5.8: Check if it's time to send daily report (non-blocking, after scan)
            try:
                check_and_send_daily_report()
            except Exception as rpt_err:
                logger.warning("  Daily report check failed: %s", rpt_err)
            ...
            # v5.7: Catch SystemExit — yfinance/pandas can call sys.exit()
            scan_crash_count += 1
            logger.error("")
            logger.error("  SCAN #%d — SystemExit caught (code=%s) — recovering (crash #%d)", sc, se.code, scan_crash_count)
            ...
            try:
                ...
                pass
            recovery_wait = min(60 + scan_crash_count * 10, 300)
            logger.info("  Auto-recovering in %d seconds...", recovery_wait)
            time.sleep(recovery_wait)
            continue
        except Exception as scan_err:
            # ═══════════════════════════════════════════════════════════════
            # v5.7 SELF-HEALING: Catch ANY error, log it, and continue
            ...
            # v5.7: Save state before recovery sleep
            try:
                save_scanner_state(sc, scan_crash_count)
            except Exception:
                pass
    ...
    # v5.8: Ensure 4 PM report fires at market close
    try:
        check_and_send_daily_report()
    except Exception as rpt_err:
        logger.warning("  Daily report check at close failed: %s", rpt_err)
    ...
    cleanup_old_data()
    cleanup_disk_space()  # v5.7: Final disk cleanup before sleeping
...
if __name__ == "__main__":
    ...
    logger.info("")
    logger.info("╔═══════════════════════════════════════════════════════════════╗")
    logger.info("║    MARKET SCANNER v5.9 — SELF-HEALING MASTER LOOP           ║")
    ...
    while True:
        try:
            ...
            # v5.7: Catch SystemExit in master loop — scanner NEVER dies
            fatal_crash_count += 1
            logger.error("")
            logger.error("  FATAL SystemExit #%d — exit code: %s", fatal_crash_count, se.code)
            logger.exception("  Full traceback:")
            try:
                clear_yfinance_cache()
                _patch_yf_tz_cache()
                ...
                pass
            try:
                save_scanner_state(0, fatal_crash_count)
            except Exception:
                pass
            recovery_wait = min(60 + fatal_crash_count * 30, 600)
            logger.info("  Auto-restarting in %d seconds (crash #%d)...", recovery_wait, fatal_crash_count)
            time.sleep(recovery_wait)
            continue
